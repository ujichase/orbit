...
impl Program {
        ...
        I: Iterator<Item = Token<SystemVerilogToken>>,
    {
        ...
        let (_, _, d_refs) = VerilogSymbol::parse_module_declaration(tokens)?;
        refs.extend(d_refs);

        // parse until finding `endprogram`
        while let Some(t) = tokens.next() {
            if t.as_type().is_eof() == true {
                return Err(SystemVerilogError::ExpectingKeyword(Keyword::Endprogram));
